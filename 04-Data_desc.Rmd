...
It will be important to be able to retrieve this data later on. We will save the "test_wells" dataframe so that we can retrieve this data again in the next section. Additionally, we will need the elevations for each well in order to calculate water table elevations in the next section. These elevations were estimated manually from Google Earth.
```{r test_wells}
